...
import type { COTable } from "../../interface";
import { toast } from "react-toastify";

interface ChangeOrderTableProps { coId: string; }
interface COFormRow {
  description: string;
  referenceDoc: string;
  elements: string;
...
const CoTable = ({ coId }: ChangeOrderTableProps) => {
  const [loading, setLoading] = useState(true);
  const { control, handleSubmit, watch, reset } = useForm<{ rows: COFormRow[] }>({
    defaultValues: { rows: [{ description: "", referenceDoc: "", elements: "", QtyNo: "0", hours: "0", cost: "0", remarks: "" }] },
  });
  ...
  useEffect(() => { fetchTableRows(); }, [coId]);

  const onSubmit = async (data: { rows: COFormRow[] }) => {
    try {
            ...
            <tbody className="divide-y divide-gray-100">
              {fields.map((field, index) => (
                <tr key={field.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-2"><Controller name={`rows.${index}.QtyNo`} control={control} render={({ field }) => <Input {...field} type="number"  />} /></td>
                  <td className="p-2"><Controller name={`rows.${index}.hours`} control={control} render={({ field }) => <Input {...field} type="number"  />} /></td>
                  <td className="p-2"><Controller name={`rows.${index}.cost`} control={control} render={({ field }) => <Input {...field} type="number"  />} /></td>
