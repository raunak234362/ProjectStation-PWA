...
const ProjectAnalyticsDashboard: React.FC<ProjectAnalyticsDashboardProps> = ({
  ...
  const fetchData = async () => {
    try {
      ...
      if (tasksRes?.data) {
        ...
        setTasks(allTasks.filter((t: any) => t.project_id === projectId));
      }
  ...
  const handleDownload = () => {
    ...
    link.setAttribute(
      "download",
      `project_analytics_${projectId}_${new Date().toISOString().slice(0, 10)}.csv`,
    );
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <SummaryCard
          ...
          value={
            Object.keys(tasksByMilestone).filter((id) => id !== "unassigned")
              .length
          }
          color="purple"
        />
