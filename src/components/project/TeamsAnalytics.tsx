...
const TeamsAnalytics = ({
  ...
  const [subTab, setSubTab] = useState<"team" | "manager">("team");
  const [managerBias, setManagerBias] = useState<any>(null);
  const [loadingBias, setLoadingBias] = useState(false);

  const [adminAnalytics, setAdminAnalytics] = useState<any>(null);
  const [adminTrendline, setAdminTrendline] = useState<any>(null);

  const [measResult, setMeasResult] = useState<any>(null);
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-50 p-6 rounded-2xl border border-black/5">
                      ...
                      <span
                        className={`text-xs font-black uppercase tracking-widest px-3 py-1 rounded-full ${managerBias.interpretation === "BALANCED" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                      >
              ...
              <div className="space-y-4">
                {measResult ? (
                  <div className="animate-in slide-in-from-bottom-2 fade-in duration-300">
                    <div className="p-5 bg-linear-to-br from-green-50 to-emerald-100/50 rounded-2xl border border-green-200 mb-6 text-center shadow-sm">
                  ...
                  <>
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-3 px-8 py-5 bg-black text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-black/90 transition-all shadow-medium group active:scale-[0.98] disabled:opacity-50"
                    >
                      {measLoading ? (
                ...
                <div className="bg-gray-50 p-6 rounded-3xl border border-black/5 flex flex-col">
                  ...
                  <div className="flex-1 space-y-2 h-[15vh] overflow-auto pr-2 custom-scrollbar">
                    {adminAnalytics.taskSummary &&
                      ...
                      adminAnalytics.taskSummary.map((ts: any, idx: number) => (
                        <div
                          key={idx}
