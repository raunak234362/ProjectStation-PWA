...
const CoordinationDrawingDetails = ({ drawingId, onClose, onUpdate }: CoordinationDrawingDetailsProps) => {
  ...
  const handleOpenFile = (file: any) => {
    openFileSecurely('coordinationDrawing', drawingId, file.id);
  };

  const handleDownloadFile = (file: any) => {
    downloadFileSecurely('coordinationDrawing', drawingId, file.id, file.originalName);
  };

  const handleShareFile = (file: any) => {
    shareFileSecurely('coordinationDrawing', drawingId, file.id);
  };
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                  ...
                  <span className="text-black">{drawing.user?.firstName} {drawing.user?.lastName}</span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                  <span className="text-gray-400">Username:</span>
                  <span className="text-black">{drawing.user?.username || 'ADMIN'}</span>
                </div>
              </div>
            </div>
