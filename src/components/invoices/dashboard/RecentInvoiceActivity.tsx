...
const RecentInvoiceActivity: React.FC<RecentActivityProps> = ({ invoices }) => {
  ...
  const activities = React.useMemo(() => {
    return [...invoices]
      ...
      .map((inv) => {
        const isPaid =
          inv.paymentStatus === true || inv.paymentStatus === "Paid";
        ...
        const date = new Date(
          inv.updatedAt || inv.createdAt || inv.invoiceDate,
        );
          ...
          activities.map((activity, index) => (
            <div key={index} className="flex gap-4">
              <div className="shrink-0 relative">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${activity.color} z-10 relative`}
                >
