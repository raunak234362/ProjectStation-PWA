        ...
        <div className="flex items-center justify-between px-10 py-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 shrink-0">
          ...
          <div className="flex items-center gap-4">
            <Button
              ...
              className="bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl shadow-green-100 dark:shadow-none transition-all active:scale-95 border-none"
            >
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
          {fabricator.branches && fabricator.branches.length > 0 ? (
              ...
              {fabricator.branches.map((branch: Branch) => (
                ...
                >
                  {branch.isHeadquarters && (
                    <div className="absolute top-0 right-0 px-4 py-1 bg-green-500 text-white text-[8px] font-black uppercase tracking-[0.2em] rounded-bl-2xl">
                      Corporate HQ
                    </div>
                  )}
                  ...
                  <h3 className="text-xl font-black text-slate-800 dark:text-white mb-2 truncate group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">
                    {branch.name}
                  </h3>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
                    <span
                      className={`text-[10px] font-black uppercase tracking-widest ${branch.isHeadquarters ? "text-green-600" : "text-slate-300 dark:text-slate-600"}`}
                    >
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-400 dark:text-slate-500">
              ...
              <Button
                onClick={handleOpenAddBranch}
                className="bg-slate-900 dark:bg-slate-700 hover:bg-black dark:hover:bg-slate-600 text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border-none"
              >
