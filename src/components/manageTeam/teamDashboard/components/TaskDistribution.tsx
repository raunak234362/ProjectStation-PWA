  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] border border-black/5 shadow-soft mb-12 h-full">
      <h3 className="text-2xl font-black text-black mb-8 flex items-center gap-3 uppercase tracking-tight">
        ...
        Task Distribution
        <span className="text-xs font-black text-[#4b8a2e] ml-auto bg-green-50 px-3 py-1.5 rounded-full border border-[#6bbd45]/20 uppercase tracking-widest">Task Counts</span>
      </h3>
          ...
          return (
            <div key={index} className="flex items-center justify-between p-4 hover:bg-gray-50/50 rounded-2xl border border-transparent hover:border-black/5 transition-all group">
              <div className="flex items-center gap-4">
                ...
                <span className="text-black/60 font-bold text-base tracking-tight group-hover:text-black">{item.label}</span>
              </div>
              <span className="text-black font-black bg-white border border-black/10 px-5 py-2 rounded-xl text-base shadow-sm group-hover:border-black/20 transition-all">
                {item.value || 0}
              </span>
            </div>
          )
