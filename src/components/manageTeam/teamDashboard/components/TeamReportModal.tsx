...
import { FileText, Download, User, Search, Award, Clock, CheckCircle2, AlertTriangle } from "lucide-react";
...
const TeamReportModal: React.FC<TeamReportModalProps> = ({
  ...
  const formatDecimalToHoursMinutes = (val: number) => {
  ...
  const processedMembers = useMemo(() => {
    ...
    return members.map((member) => {
      ...
      // 1. Assigned Hours
      const assignedHours = (member.tasks || []).reduce((sum: number, task: any) => {
        let h = 0;
      ...
      // 3. Completed Tasks
      const completedTasksCount = (member.tasks || []).filter((task: any) =>
        ["COMPLETED", "COMPLETE", "USER_FAULT", "VALIDATE_COMPLETED"].includes(task.status?.toUpperCase())
      ).length;

      // 4. Efficiency
      const completedTasks = (member.tasks || []).filter(
        (task: any) => ["COMPLETED", "COMPLETE"].includes(task.status?.toUpperCase())
      );

      const efficiencyAssigned = completedTasks.reduce((sum: number, task: any) => {
        let h = 0;
        if (task.allocationLog && task.allocationLog.allocatedHours !== undefined && task.allocationLog.allocatedHours !== null) {
          h = safeParseFloat(task.allocationLog.allocatedHours);
        } else if (task.hours !== undefined && task.hours !== null) {
      ...
      const sortedTasks = [...(member.tasks || [])].sort((a: any, b: any) => {
  ...
  const generatePDFForMember = (doc: any, m: any, startY = 15) => {
    // Primary Branding Colors: Soft Green theme
    const primaryColor: [number, number, number] = [107, 189, 69]; // #6bbd45 (Tekla green)
    const textColor: [number, number, number] = [30, 30, 30];
    const lightBg: [number, number, number] = [244, 252, 240];
    ...
    const tableRows = (m.tasks || []).map((t: any) => [
      t.project?.name || "N/A",
      ...
      formatDecimalToHoursMinutes(
        (t.workingHourTask || []).reduce((sum: number, entry: any) => {
          if (entry.duration_seconds) return sum + entry.duration_seconds / 3600;
          return sum + (entry.duration || 0) / 60;
        }, 0)
      ) + " hrs",
  ...
  const handleExportExcelSingle = () => {
    ...
    try {
      ...
      activeMember.tasks.forEach((task: any, idx: number) => {
        const taskWorked = (task.workingHourTask || []).reduce((sum: number, entry: any) => {
          if (entry.duration_seconds) return sum + entry.duration_seconds / 3600;
          return sum + (entry.duration || 0) / 60;
        }, 0);
      ...
      const maxColWidth = sheetData.reduce((widths: number[], row: any[]) => {
  ...
  const handleExportExcelAll = () => {
    ...
    try {
      ...
      processedMembers.forEach((m: any) => {
        ...
        m.tasks.forEach((task: any, idx: number) => {
          const taskWorked = (task.workingHourTask || []).reduce((sum: number, entry: any) => {
            if (entry.duration_seconds) return sum + entry.duration_seconds / 3600;
            return sum + (entry.duration || 0) / 60;
          }, 0);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-[96%] max-w-[95vw] h-[90vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Active Period Filter: <span className="text-[#6bbd45] font-bold">{getReadableDateFilter()}</span>
            </p>
          </div>
            ...
            <div className="p-4 border-b border-gray-100 shrink-0">
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-[#6bbd45]/20 focus:border-[#6bbd45] transition-all placeholder:text-gray-400"
                />
              </div>
            </div>
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs uppercase shrink-0
                        ...
                        {m.fullName.split(" ").map((w: string) => w[0]).join("").substring(0, 2)}
                      </div>
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0 bg-green-50/20">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-800 uppercase">{activeMember.fullName}</h3>
                ...
                <div className="p-6 grid grid-cols-2 lg:grid-cols-4 gap-4 shrink-0 border-b border-gray-100 bg-gray-50/20">
                  ...
                  <div className="p-4 bg-white border border-gray-200 rounded-xl shadow-sm flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-green-600 mt-1.5 leading-none">{activeMember.completedTasks} / {activeMember.totalTasks}</p>
                    </div>
                  </div>
                    ...
                    <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                      <table className="w-full text-left border-collapse bg-white">
                        ...
                        <tbody className="divide-y divide-gray-100 text-xs font-medium text-gray-700">
                          {activeMember.tasks.map((task: any, idx: number) => {
