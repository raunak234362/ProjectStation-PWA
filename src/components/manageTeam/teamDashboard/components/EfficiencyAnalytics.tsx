...
interface EfficiencyAnalyticsProps {
    ...
    onTeamSelectionChange: (selectedIds: string[]) => void;
    timeFilter: string;
...
// Custom Control to look like a button
const Control = ({ children, ...props }: any) => {
    return (
        <components.Control {...props} className="!border-0 !shadow-none !bg-transparent">
            <div className="flex items-center gap-1 bg-gray-100 hover:bg-gray-200 transition-colors px-3 py-1.5 rounded-lg cursor-pointer text-xs font-medium text-gray-700">
                <span>Compare Teams</span>
                <span className="text-lg leading-none">+</span>
            </div>
            {/* Hide default rendering which includes the input/placeholder in a way we don't want */}
...
const EfficiencyAnalytics: React.FC<EfficiencyAnalyticsProps> = ({
    ...
    const handleTeamChange = (selectedOptions: any) => {
        onTeamSelectionChange(selectedOptions ? selectedOptions.map((o: any) => o.value) : []);
    };

    const selectedTeamOptions = teamOptions.filter(opt => selectedTeams.includes(opt.value));
                    ...
                    >
                        <defs>
                            {selectedTeams.map((teamId, index) => (
                                <linearGradient key={teamId} id={`colorEff-${teamId}`} x1="0" y1="0" x2="0" y2="1">
                        ...
                        <YAxis
                            ...
                            label={{ value: 'Efficiency %', angle: -90, position: 'insideLeft', style: { fill: '#9ca3af', fontSize: 12 } }}
                        />
                        <Tooltip
                            contentStyle={{
                                backgroundColor: "#fff",
                            ...
                            labelStyle={{ color: "#374151", marginBottom: "4px", fontWeight: 600, fontSize: '12px' }}
