...
const CDNetworkOverview: React.FC<CDNetworkOverviewProps> = ({ designers, stateData, onSelect }) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-[500px]">
            ...
            >
                <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        <h3 className="text-lg font-bold text-gray-800">Connection Designer Directory</h3>
                        <p className="text-xs text-gray-400 mt-1">Click to view details • Hover to see states</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                    {filteredDesigners.map((designer) => {
                        ...
                        if (Array.isArray(designer.state)) states = designer.state;
                            ...
                            try { states = designer.state.startsWith('[') ? JSON.parse(designer.state) : [designer.state]; } catch { states = [designer.state]; }
                        }
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-semibold text-gray-800">{designer.name}</h4>
                                    ...
                                    {hoveredId === (designer.id || designer._id) && (
                                        ...
                                        >
                                            <div className="flex items-center gap-2 mb-2 pb-2 border-b border-gray-50">
                                                ...
                                                <span className="text-xs font-bold text-gray-700">Coverage Area</span>
                                            </div>
                                            <div className="flex flex-wrap gap-1.5">
                                                {states.length > 0 ? states.slice(0, 8).map((s, i) => (
                                                    <span key={i} className="text-[10px] bg-green-50 text-green-700 px-2 py-0.5 rounded-md border border-green-100">
                                                        {s}
                                                    </span>
                                                )) : <span className="text-xs text-gray-400 italic">No specific states listed</span>}
                                                {states.length > 8 && (
                                                    <span className="text-[10px] text-gray-400 pl-1">+{states.length - 8} more</span>
                                                )}
                                            </div>
            ...
            >
                <h3 className="text-lg font-bold text-gray-800 mb-6">State Distribution</h3>
                <div className="flex-1 min-h-[250px] relative">
                    ...
                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none mb-8">
                        <span className="text-2xl font-bold text-gray-800">{stateData.reduce((a, b) => a + b.count, 0)}</span>
