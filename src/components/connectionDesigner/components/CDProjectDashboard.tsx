  ...
  const dashboardStats = useMemo(() => {
    ...
    projects.forEach((p) => {
      ...
      if (Array.isArray(p.rfi)) {
        pendingRFI += p.rfi.filter(
          (r) => r.status === "PENDING" || r.status === false,
        ).length;
      }
      // Pending Submittals
      if (Array.isArray(p.submittals)) {
        pendingSubmittals += p.submittals.filter(
          (s) => s.status === "PENDING",
        ).length;
      }
      // Pending Change Orders
      const cos = Array.isArray(p.changeOrders) ? p.changeOrders : [];
      pendingChangeOrders += cos.filter(
        (co: any) =>
          co.isAproovedByAdmin === "PENDING" || co.status === "PENDING",
      ).length;
    });
    ...
    <div className="flex flex-col w-full space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="bg-white p-8 rounded-[40px] shadow-soft border border-green-500/10">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-xl  text-gray-800 uppercase tracking-tight">
              ...
              {projects
                ...
                .map((project) => (
                  <div
                    ...
                    className="p-6 rounded-[32px] border border-green-500/10 hover:border-green-200 hover:bg-green-50/20 transition-all cursor-pointer group shadow-sm hover:shadow-md"
                  >
                    <div className="flex justify-between items-start mb-6">
                      <div className="min-w-0">
                        <h4 className=" text-gray-800 group-hover:text-green-700 transition-colors truncate">
                          {project.name}
                        </h4>
                      ...
                      <span className="px-3 py-1 rounded-lg bg-green-50 text-green-600 text-[9px]  uppercase tracking-widest border border-green-100">
                        {project.stage || "Design"}
                      </span>
                    </div>
                    <div className="flex gap-6 mt-4 pt-4 border-t border-green-50/50">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm  text-gray-700">
                          {Array.isArray(project.changeOrders)
        ...
        <div className="lg:col-span-1">
          <UpcomingSubmittals
            pendingSubmittals={allPendingSubmittals}
            hideTabs={true}
          />
        </div>
      </div>
