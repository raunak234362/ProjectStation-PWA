...
const EstimationDashboard = () => {
    ...
    const [topFabricator, setTopFabricator] = useState<any>(null);
    ...
    const calculateStats = (data: any[]) => {
        ...
        const totalAwarded = data.filter((e: any) =>
            e.status?.toUpperCase() === "AWARDED" || e.status?.toUpperCase() === "COMPLETED"
        ).length;
        ...
        // 4. Top Fabricator
        calculateTopFabricator(data);
    };

    const calculateTopFabricator = (data: any[]) => {
        const fabMap: Record<string, { name: string, count: number, hours: number }> = {};

        data.forEach((e: any) => {
            // Fabricator might be an object or ID
            // AllEstimation interface suggests 'fabricator.fabName'
            const fabId = e.fabricatorId || e.fabricator?.id;
            const fabName = e.fabricator?.fabName || e.fabricatorName || "Unknown";

            if (fabId || fabName !== "Unknown") {
                const key = fabId || fabName; // Use ID if valid, else unique name
                if (!fabMap[key]) {
                    fabMap[key] = { name: fabName, count: 0, hours: 0 };
                }
                fabMap[key].count += 1;
                const hours = e.estimatedHours || e.finalHours || e.totalAgreatedHours || 0;
                fabMap[key].hours += Number(hours);
            }
        });
        ...
        Object.values(fabMap).forEach(fab => {
            if (fab.count > maxCount) {
                maxCount = fab.count;
                max = fab;
            }
        });
        ...
        if (max) {
            setTopFabricator({
                name: (max as any).name,
                projectCount: (max as any).count,
                totalHours: (max as any).hours
            });
        }
    };
    ...
    const processChartData = (data: any[]) => {
        ...
        data.forEach((e: any) => {
            ...
            const monthYear = date.toLocaleString('default', { month: 'short', year: '2-digit' }); // e.g. "Jan 23"
        ...
        const sortedData = Object.values(monthlyData).sort((a: any, b: any) => a.sortKey.localeCompare(b.sortKey));
