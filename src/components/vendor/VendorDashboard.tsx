...
const DashboardSkeleton = () => (
  <div className="p-6 space-y-6 animate-pulse">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="h-28 bg-green-50 rounded-2xl"></div>
      ))}
    </div>
    {/* Network Overview Skeleton */}
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[500px]">
      <div className="lg:col-span-2 bg-green-50 rounded-2xl"></div>
      <div className="lg:col-span-1 bg-green-50 rounded-2xl"></div>
    </div>
    {/* Bottom Skeleton */}
    <div className="h-48 bg-green-50 rounded-2xl"></div>
  </div>
);
        ...
        {selectedDesignerId && (
          ...
          >
            <motion.div
              ...
              className="bg-white border border-green-500/10 rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative"
            >
              <button
                onClick={() => setSelectedDesignerId(null)}
                className="absolute top-4 right-4 p-2 bg-green-50 hover:bg-green-100 rounded-full transition-colors z-10"
              >
