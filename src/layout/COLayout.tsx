...
const COLayout = () => {
  ...
  return (
    <div className="w-full overflow-y-hidden overflow-x-hidden project-component-container">
      ...
      <div className={`flex-1 min-h-0 bg-white p-2 rounded-b-2xl overflow-y-auto ${!showAddButton ? "pt-4" : ""}`}>
        {activeTab === "allCO" && <div><AllCO /></div>}
        {showAddButton && activeTab === "addCO" && <div><AddCO onSuccess={() => setActiveTab("allCO")} /></div>}
      </div>
    </div>
  );
};
